...
function initEmployeeDropdown() {
    ...
    empChecks.forEach(cb => {
        cb.onchange = null;
        cb.addEventListener('change', updateEmployeeDropdownText);
    });

    if (selectAll) {
        selectAll.onchange = null;
        selectAll.addEventListener('change', function() {
            empChecks.forEach(cb => cb.checked = this.checked);
            updateEmployeeDropdownText();
        });
    }
...
function updateEmployeeDropdownText() {
    const btnText = document.getElementById('employeeDropdownText');
    const empChecks = document.querySelectorAll('.emp-check');
    if (!empChecks) {
        btnText.innerText = 'Choose Employees';
        return;
    }

    const empArr = Array.from(empChecks);
    const checked = empArr.filter(cb => cb.checked);
    const selectAll = document.getElementById('select_all');

    if (checked.length === 0) {
        btnText.innerText = 'Choose Employees';
        if (selectAll) selectAll.checked = false;
    } else if (checked.length === empArr.length) {
        btnText.innerText = 'All Employees Selected';
        if (selectAll) selectAll.checked = true;
    } else if (checked.length === 1) {
        const label = checked[0].nextElementSibling ? checked[0].nextElementSibling.innerText.trim() : '1 employee selected';
        btnText.innerText = label;
        if (selectAll) selectAll.checked = false;
    } else {
        btnText.innerText = checked.length + ' employees selected';
        if (selectAll) selectAll.checked = false;
    }
}
</script>

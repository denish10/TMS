...
$filter_task_id = isset($_GET['id']) ? intval($_GET['id']) : null;
...
if ($filter_task_id) {
  ...
  $detailQuery = "
    ...
    WHERE ta.users_id = $user_id AND ta.task_id = $filter_task_id
    ORDER BY ta.record_id DESC
    LIMIT 1
  ";

  $detailResult = mysqli_query($conn, $detailQuery);
  $task = $detailResult ? mysqli_fetch_assoc($detailResult) : null;
?>
...
<div class="container mt-4" >
  ...
  <?php if ($task) { ?>
    <div class="card shadow p-4" style="margin-left: 204px; ">
      ...
      <div class="mt-3">
        <a href="task_manage.php" class="btn btn-secondary">Back</a>
      </div>
    </div>
  <?php } else { ?>
    <div class="alert alert-danger text-center">⚠️ Task not found.</div>
    <div class="text-center">
      <a href="task_view.php" class="btn btn-secondary">Back</a>
    </div>
  <?php } ?>
</div>
